...
const inputbarberName = document.getElementById("inputbarberName");
...
async function updateOptions(depId) {
    ...
    response = await fetch(`http://localhost:8080/barbers?depId=${depId}`);
    let barbers = await response.json();

    strs = [];
    strs.push(`<option hidden disabled selected value="0">Выберите парикхмахера</option>`);
    for (let barber of barbers) {
        strs.push(`<option value="${barber.id}">${barber.barbername}</option>`)
    }

    inputbarberName.innerHTML = strs.join('');
}
